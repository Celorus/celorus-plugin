...
1. Declare the surface class in `<head>` — `<meta name="celorus-surface" content="...">`
   (collateral | command | tier1-artifact | tier2-embedded | app | deck | marketing | banner |
   email). The composition lint keys off it; full contract in `references/COMPOSITION.md`.
2. Inline `assets/foundation-block.css` into a `<style>` tag (tokens, reset, type baseline,
   light + dark themes, legacy aliases). For data deliverables (figures, citations, status
   pills) also inline `assets/collateral.css`.
3. Bind every style to a CSS variable — `var(--semantic-*)` / `var(--component-*)` (primitives
...
4. **Theme activation.** The foundation block ships both themes but dark only activates via a
...
5. **Fonts.** Standalone files may `@import` the Google Fonts set (Source Serif 4, IBM Plex
...
6. Page skeleton (standalone pages only; embedded surfaces differ — see below): charcoal topbar
   with the wordmark (see step 7 — it is TWO images, not one) + a meta tag
...
7. **Wordmark — ship BOTH marks and swap on the theme.** The topbar sits on
   `--semantic-color-bg-inverse`, which is charcoal in light and **bone in dark**. The wordmark
   SVGs are fixed-colour assets and do not flip with it, so a lone white mark scores **1.13:1
   in dark — invisible**. `tools/composition-lint.mjs` fails the build if you ship one alone.

   ```html
   <img class="wm wm-light" src="assets/celorus-mono-white.svg" alt="Celorus">
   <img class="wm wm-dark"  src="assets/celorus-primary.svg"    alt="Celorus">
   ```
   ```css
   .wm{height:22px;width:auto;display:block}     /* sizing HERE — see the trap below */
   .wm-dark{display:none}
   [data-theme="dark"] .wm-light{display:none}
   [data-theme="dark"] .wm-dark{display:block}
   ```
   ⚠️ **Keep the sizing in `.wm`, never inline on the `<img>`.** An inline `display:block`
   outranks the stylesheet's `display:none`, so both marks render side by side and the fix
   looks like it failed. A sibling rule like `.brand img{display:block}` does the same —
   scope such rules to `.brand .wm` and set height only.

   Favicon: `assets/celorus-favicon.svg`; where a host only accepts
...
7a. **Anything else ON the inverse surface takes an on-inverse token, never a raw primitive.**
   `bg.inverse` flips; `--primitive-color-*` cannot. Use `--semantic-color-fg-on-inverse`
   (primary), `-on-inverse-muted` (secondary), `-on-inverse-faint` (labels/metadata) and
   `--semantic-color-accent-on-inverse` (kickers, rules, accents). Binding these to
   `--primitive-color-petrol-300` / `--primitive-color-dark-ink-*` froze them light-on-bone
   (2.30:1 and 1.19:1) across all eight templates until 2026-08-10.
   A surface that is *deliberately always charcoal* (e.g. the `.kfi-grid.on-dark` weight
   strip) is the opposite case — give it primitives and do **not** mix in `bg.inverse`.
8. Interactive elements keep visible focus (`:focus-visible` ring is in the foundation block).
...
- **Tier 1 — full-page hosted artifact** (claude.ai artifact viewer, any hosted single-file
  ...
  (Step 5). Do NOT apply the compact rules here — a full-page artifact with stripped chrome
...
- **Tier 2 — truly embedded** (plugin panels, iframes, chat-rendered widgets, ≤480px cards):
  ...
  placed once (header corner or footer); theme follows the host (the Step-4 bridge); fonts
...
- BOTH themes **rendered and looked at** — not just "the bridge is present". Stamp
  `data-theme="dark"` and open it. A lint pass is not a both-themes check: the eight
  templates passed every lint while the dark topbar showed an invisible wordmark
  (1.13:1) and a washed-out kicker (2.30:1). Check the topbar/hero specifically —
  `bg.inverse` INVERTS there (charcoal → bone), so anything on it that did not flip
...
- Surface declared and the composition lint passes: live system
  `node design-system/tools/composition-lint.mjs <file>`; bundled fallback
  `node assets/composition-lint.mjs <file>` (same file).
